...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        df = df.groupby(['Launch Site','class']).size().reset_index(name = 'count')
...
def scatter(entered_site,payload):
    ...
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df1,x='Payload Mass (kg)',y='class',
                         color='Booster Version Category',title='Correlation between successful launches and Payload mass for all sites')
        return fig
    else:
        df1 = filtered_df1[filtered_df1['Launch Site']==entered_site]
        fig = px.scatter(df1,x='Payload Mass (kg)',y='class',
                         color='Booster Version Category',title=f"Correlation between successful launches and Payload mass for site {entered_site}")
        return fig
